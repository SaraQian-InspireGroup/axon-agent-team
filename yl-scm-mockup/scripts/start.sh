...
if [[ -f "$PID_FILE" ]]; then
  ...
  if kill -0 "$old_pid" 2>/dev/null; then
    echo "Mockup frontend already running (pid=$old_pid). Use scripts/stop.sh first."
    exit 1
  fi
  rm -f "$PID_FILE"
fi
...
if [[ ! -d "$ROOT/node_modules" ]]; then
  echo "Dependencies not installed. Run from yl-scm-mockup/:"
  echo "  npm install"
  exit 1
fi
...
for _ in $(seq 1 30); do
  if ! kill -0 "$pid" 2>/dev/null; then
    echo "Mockup frontend failed to start. Last log lines:"
...
echo "Mockup frontend started (pid=$pid)"
